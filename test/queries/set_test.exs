defmodule Queries.SetTest do
  ...
  describe "SET" do
    test "allows updating a single property" do
      ...
      expected = "MATCH (p:Person {name:\"Andy\"}) " <> "SET p.name = \"Bob\" " <> "RETURN p.name"
    ...
    test "allows updating a single property to NULL" do
      ...
      expected = "MATCH (p:Person {name:\"Andy\"}) " <> "SET p.name = NULL " <> "RETURN p.name"
    ...
    test "allows fragments inside queries" do
      ...
      expected =
        "MATCH (p:Person {name:\"Andy\"}) " <> "SET p.age = toString(p.age) " <> "RETURN p.name"
    ...
    test "allows using a map to set multiple properties at once" do
      ...
      expected =
        "MATCH (p:Person {name:\"Andy\"}) " <> "SET p = {age:35,name:\"Bob\"} " <> "RETURN p.name"
    ...
    test "allows removing all properties with an empty map" do
      ...
      expected = "MATCH (p:Person {name:\"Andy\"}) " <> "SET p = {} " <> "RETURN p.name"
    ...
    test "allows multiple properties using a comma separator" do
      ...
      expected =
        "MATCH (p:Person {name:\"Andy\"}) " <>
          "SET p.name = \"bob\", p.age = 23 " <> "RETURN p.name"
    ...
    test "allows setting a new label to a node" do
      ...
      expected = "MATCH (p:Person {name:\"Andy\"}) " <> "SET p:Teacher " <> "RETURN p.name"
    ...
    test "allows setting multiple labels to a node" do
      ...
      expected =
        "MATCH (p:Person {name:\"Andy\"}) " <> "SET p:Teacher:Scientist " <> "RETURN p.name"
    ...
    test "allows updating specific properties" do
      ...
      expected = "MATCH (p:Person {name:\"Andy\"}) " <> "SET p += {age:35} " <> "RETURN p.name"
